...
function grd_zero.func_keys(x, y, z)
  if y == 5 then
  ...
  if y == 9 and x == 15 then
    mod_d = z == 1 and true or false
    if mod_c and z == 1 then
      params:set("glb_midi_in_destination", 1)
    end
  end
  if y == 9 and x == 16 then
    mod_b = z == 1 and true or false
  end
  if (y == 9 or y == 11) and x > 7 and x < 9 and not sample_view then
    heldkey = heldkey + (z * 2 - 1)
  end
  if y == 10 and ((x > 3 and x < 8) or (x > 9 and x < 14)) and not sample_view then
    heldkey = heldkey + (z * 2 - 1)
  end

  if (y == 10 or y == 11) and (x > 3 and x < 14) and sample_view then
    grd_zero.sampl_grid(x, y, z)
  end
  ...
  if y == 13 and x == 16 and retrig_mode then
    gkey[x][y].active = z == 1 and true or false
    get_repeat_rate()
  end

  if y == 14 and x == 16 then
    if retrig_mode then
      gkey[x][y].active = z == 1 and true or false
      get_repeat_rate()
    else
  ...
  if y == 16 and x == 16 and retrig_mode then
    gkey[x][y].active = z == 1 and true or false
    get_repeat_rate()
  end
...
function grd_zero.main_grid(x, y, z)
  if z == 1 then
    if y == 7 then
      -- set interval_focus
      if x < 4 or x > 13 then
    ...
    elseif y == 8 then
      ...
      if x < 4 or x > 13 then
        ...
        if (mod_a or mod_b) then
          params:set("voice_mute"..i, voice[i].mute and 1 or 2)
        elseif mod_c and not mod_a then
          params:set("glb_midi_in_destination", i + 1)
        elseif mod_d then
          params:delta("midi_panic"..i, 1)
        elseif not voice[i].mute then
          key_focus = i
          voice_focus = i
          notes.held = {}
    ...
    elseif y == 9 and not sample_view then
...
function grd_zero.pattern_grid(x, y, z)
  ...
  if y < 4 and x > 4 and x < 13 then
    ...
    elseif pattern_length_config and z == 1 then
      -- set bar num
      if y < 3 then
        params:set("patterns_beatnum"..pattern_focus, i + 8 * (y - 1))
        if pattern[pattern_focus].manual_length then
          pattern[pattern_focus].manual_length = false
          --pattern[pattern_focus].length = pattern[pattern_focus].meter * pattern[pattern_focus].beatnum
          --pattern[pattern_focus]:set_length(pattern[pattern_focus].length)
...
function grd_zero.sampl_grid(x, y, z)
  if y > 9 and y < 12 and x > 3 and x < 14 then
    --gkey[x][y].active = z == 1 and true or false
    local bank = math.ceil((x - 3) / 2)
    local slot = (x - 3) - (bank - 1) * 2 + (y - 10) * 2
    if z == 1 then
...
function grd_zero.scale_grid(x, y, z)
  if y > 12 and x > 2 and x < 15 then
    ...
    if z == 1 then
      ...
      local note = (x - 2) + ((16 - y) * sc_iy) + (notes.oct_key + 3) * octave
...
function grd_zero.chrom_grid(x, y, z)
  if y > 12 and x > 2 and x < 15 then
    ...
    local note = (root + x * ch_ix) + ch_iy * (16 - y)
    gkey[x][y].active = z == 1 and true or false
    if z == 1 then
      -- keep track of held notes
      gkey[x][y].note = note
...
function grd_zero.chord_grid(x, y, z)
  if y > 12 and y < 16 and x > 2 and x < 15 then
    ...
    elseif z == 0 then
      if #chord[i] > 0 and not (gkey[x][13].active or gkey[x][14].active or gkey[x][15].active) then
        for index, value in ipairs(chord[i]) do
          local e = {t = eKEYS, p = pattern_focus, i = key_focus, note = value, action = "note_off"} event(e)
          notes.held = {}
        end
...
function grd_zero.drum_grid(x, y, z)
  if y > 13 and x > 2 and x < 15 then
    local note = (params:get("drum_root_note") + 12 * notes.oct_key) + (x - 4)
    local vel = y == 16 and params:get("drum_vel_hi") or (y == 15 and params:get("drum_vel_mid") or params:get("drum_vel_lo"))
    gkey[x][y].active = z == 1 and true or false
    if z == 1 then
      last_velocity = vel
      if not key_repeat then
...
function grd_zero.draw()
  ...
  for i = 1, 8 do
    ...
    elseif pattern[i].play == 1 then
      g:led(i + 4, 7, p[i].key_flash and 15 or 12)
  ...
  if params:get("metronome_viz") == 2 then
    g:led(16, 10, flash_bar and 15 or (flash_beat and 8 or 5)) -- Q flash
  end
  -- pattern edit
  ...
  if pattern_meter_config then
    for i = 1, 8 do
      g:led(i + 4, 1, pattern[pattern_focus].manual_length and 1 or (params:get("patterns_meter"..pattern_focus) == i and 8 or 2))
      g:led(i + 4, 3, pattern_focus == i and 10 or 1)
    end
  elseif pattern_length_config then
    for i = 1, 8 do
      g:led(i + 4, 1, pattern[pattern_focus].manual_length and 1 or (params:get("patterns_beatnum"..pattern_focus) == i and 12 or 3))
      g:led(i + 4, 2, pattern[pattern_focus].manual_length and 1 or (params:get("patterns_beatnum"..pattern_focus) == i + 8 and 15 or 4))
      g:led(i + 4, 3, pattern_focus == i and 10 or 1)
    end
  elseif pattern_options_config then
    for i = 1, 8 do
      g:led(i + 4, 1, params:get("patterns_countin"..i) == 1 and 2 or (params:get("patterns_countin"..i) == 2 and 6 or 12))
    ...
    for i = 1, 8 do
      ...
      for j = 1, 3 do
        g:led(i + 4, j, p[i].load == j and ledslow or (p[i].bank == j and (p[i].count[j] > 0 and 15 + dim or 10 + dim) or (p[i].count[j] > 0 and 6 + dim or 2 + dim)))
      end
    end
  end
  -- pattern position
  ...
  for i = 1, 3 do
    ...
    if not mod_c then
      ...
      g:led(i, 8, params:get("glb_midi_in_destination") == i + 1 and 15 or 0)
      g:led(i + 13, 8, params:get("glb_midi_in_destination") == i + 4 and 15 or 0)
    end
  end
  -- keyboard options
  g:led(1, 10, params:get("keys_option"..key_focus) == 1 and 8 or 4)
  g:led(2, 10, params:get("keys_option"..key_focus) == 2 and 8 or 4)
  g:led(1, 11, params:get("keys_option"..key_focus) == 3 and 8 or 4)
  g:led(2, 11, params:get("keys_option"..key_focus) == 4 and 8 or 4)

  g:led(15, 10, key_quantize and 8 or 4)
  g:led(15, 11, sample_view and 10 or 4)
  g:led(16, 11, retrig_mode and 10 or 4)
  ...
  if sample_view then
    ...
    for i = 8, 9 do
      g:led(i, 9, 6) -- home
      g:led(i, 10, key_link and 2 or 0) -- key link
  ...
  -- int octave
  g:led(1, 13, 8 + notes.oct_int * 2)
  g:led(1, 14, 8 - notes.oct_int * 2)
  -- key octave
  g:led(1, 15, 8 + notes.oct_key * 2)
  g:led(1, 16, 8 - notes.oct_key * 2)
  -- sequencer
  if retrig_mode then
    for i = 1, 4 do
  ...
  if params:get("keys_option"..key_focus) == 1 then
    for i = 1, 12 do
      local octave = #scale_intervals[params:get("scale")] - 1
      g:led(i + 2, 13, gkey[i + 2][13].active and 15 or (((i + sc_iy * 3) % octave) == 1 and 10 or 2))
      g:led(i + 2, 14, gkey[i + 2][14].active and 15 or (((i + sc_iy * 2) % octave) == 1 and 10 or 2))
      g:led(i + 2, 15, gkey[i + 2][15].active and 15 or (((i + sc_iy) % octave) == 1 and 10 or 2))
      g:led(i + 2, 16, gkey[i + 2][16].active and 15 or ((i % octave) == 1 and 10 or 2))
    end
  elseif params:get("keys_option"..key_focus) == 2 then
    for x = 3, 14 do
      for y = 13, 16 do
        local note = (x * ch_ix - 3) + ch_iy * (16 - y)
        local st = note % 12
  ...
  elseif params:get("keys_option"..key_focus) == 3 then
    ...
    g:led(4, 16, chord_harp and 10 or 2)
    g:led(5, 16, strum_options and 15 or 2)
    if strum_options then
      g:led(params:get("arp_length") + 2, 16, 10)
    else
      g:led(6, 16, strum_direction == 1 and 4 or 1)
      g:led(8, 16, strum_rate == 0.5 and 8 or 2)
      g:led(9, 16, strum_rate == 0.02 and 8 or 2)
      for i = 1, 4 do
        g:led(i + 10, 16, chord_inversion == i - 1 and 6 or 2)
      end
    end
